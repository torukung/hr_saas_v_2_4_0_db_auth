...
const ob = () => JSON.parse(localStorage.getItem("adeptio.v232.turso.outbox") || "[]");
async function drain() { for (let i = 0; i < 60 && ob().length; i++) { await TURSO.flush(); await sleep(25); } }

(async () => {
  const errors = [];
  await TURSO.pull(); await drain();
  if (Object.keys(cloud.meta).length !== 10) errors.push("expected 10 stores in sync_meta, got " + Object.keys(cloud.meta).length);
